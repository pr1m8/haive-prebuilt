...
def create_calculator_tool() -> StructuredTool:
    ...
    def calculate(expression: str) -> Dict[str, Any]:
        ...
        try:
            ...
            safe_expr = expression.replace("__", "").replace("import", "").replace("eval", "")
...
def get_tools_for_mode(mode: SearchMode) -> List[StructuredTool]:
    ...
    base_tools = [
        ...
        TOOL_REGISTRY["load_webpage"]()
    ]
    
    if mode in [SearchMode.PRO, SearchMode.LABS]:
        base_tools.extend([
            TOOL_REGISTRY["calculator"](),
            TOOL_REGISTRY["python_interpreter"]()
        ])
    
    return base_toolsREGISTRY["query_analysis"],
...
def create_rag_generation_engine(model: ModelChoice = ModelChoice.GPT_4O) -> AugLLMConfig:
    ...
    return AugLLMConfig(
        ...
        llm_config=AzureLLMConfig(
            model=model_mapping.get(model, "gpt-4o"),
            temperature=0.3,
            max_tokens=2000
        ),
