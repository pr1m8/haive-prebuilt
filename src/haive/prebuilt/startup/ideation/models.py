...
class ProblemStatement(BaseModel):
    ...
    current_solutions: list[str] = Field(default_factory=list, description='How people currently solve this')
    pain_points: list[str] = Field(..., description='Specific pain points')
    emotional_impact: str | None = Field(None, description='How users feel about this problem')
    financial_impact: str | None = Field(None, description='Cost of the problem')
    evidence: list[str] = Field(default_factory=list, description='Evidence this problem exists')
    quotes: list[str] = Field(default_factory=list, description='Real user quotes about the problem')
    research_sources: list[str] = Field(default_factory=list, description='Sources validating the problem')
    validation_score: float = Field(0.0, ge=0.0, le=1.0, description='How well validated the problem is')
    ...
    def problem_score(self) -> float:
        ...
        frequency_scores = {'daily': 1.0, 'weekly': 0.8, 'monthly': 0.6, 'occasionally': 0.4, 'rarely': 0.2}
...
class SolutionConcept(BaseModel):
    ...
    technical_feasibility: float = Field(0.5, ge=0.0, le=1.0, description='Technical feasibility score')
    implementation_complexity: RiskLevel = Field(RiskLevel.MEDIUM)
    required_technologies: list[str] = Field(default_factory=list)
    user_journey: list[str] = Field(default_factory=list, description='Key steps in user journey')
...
class MarketResearch(BaseModel):
    ...
    market_drivers: list[str] = Field(default_factory=list, description="What's driving market growth")
    market_barriers: list[str] = Field(default_factory=list, description='Barriers to market entry')
    primary_customers: list[dict[str, Any]] = Field(default_factory=list, description='Primary customer segments')
    secondary_customers: list[dict[str, Any]] = Field(default_factory=list, description='Secondary segments')
    early_adopters: list[str] = Field(default_factory=list, description='Early adopter characteristics')
    ...
    def validate_market_sizes(self):
        ...
        if all([self.serviceable_addressable_market, self.serviceable_obtainable_market]):
...
class BusinessModelCanvas(BaseModel):
    ...
    customer_relationships: list[str] = Field(..., description='How we maintain relationships')
    ...
    unfair_advantage: str | None = Field(None, description='Sustainable competitive advantage')
...
class RiskAssessment(BaseModel):
    ...
    """
    model_config = ConfigDict(extra='forbid')
    market_risk: RiskLevel = Field(..., description='Risk of market not accepting solution')
    ...
    identified_risks: list[dict[str, Any]] = Field(default_factory=list, description='Specific risks with mitigation strategies')
    ...
    def calculate_overall_risk(self):
        ...
        risk_values = {RiskLevel.LOW: 1, RiskLevel.MEDIUM: 2, RiskLevel.HIGH: 3, RiskLevel.VERY_HIGH: 4}
        risks = [self.market_risk, self.technical_risk, self.financial_risk, self.competitive_risk, self.regulatory_risk, self.team_risk]
...
class ValidationResult(BaseModel):
    ...
    sample_size: int | None = Field(None, description='Number of participants/data points')
...
class IdeaMetrics(BaseModel):
    ...
    def calculate_scores(self):
        ...
        weights = {'problem_severity': 0.2, 'solution_uniqueness': 0.2, 'market_opportunity': 0.25, 'feasibility': 0.2, 'scalability': 0.15}
        self.overall_score = sum((getattr(self, metric) * weight for metric, weight in weights.items()))
...
class IdeaBrainstorm(BaseModel):
    ...
    participating_agents: list[str] = Field(default_factory=list, description='Agent IDs involved')
    raw_ideas: list[str] = Field(default_factory=list, description='All generated ideas')
    refined_ideas: list['StartupIdea'] = Field(default_factory=list, description='Refined idea objects')
    techniques_used: list[str] = Field(default_factory=list, description='Brainstorming techniques')
...
class StartupIdea(BaseModel):
    ...
    contributing_agents: list[str] = Field(default_factory=list, description='Agents that contributed')
    research_sources: list[str] = Field(default_factory=list, description='External sources used')
    ...
    def add_validation(self, validation: ValidationResult) -> None:
        ...
        self._validation_status[validation.method.value] = validation.validates_hypothesis or False
    ...
    def calculate_readiness(self) -> dict[str, Any]:
        ...
        readiness = {'research_completeness': len(self._research_tasks_completed) / 5, 'validation_completeness': len(self.validation_results) / 3, 'has_business_model': self.business_model is not None, 'has_risk_assessment': self.risk_assessment is not None, 'has_metrics': self.metrics is not None, 'overall_score': self.metrics.overall_score if self.metrics else 0}
        readiness['overall_readiness'] = sum([readiness['research_completeness'] * 0.3, readiness['validation_completeness'] * 0.3, readiness['has_business_model'] * 0.2, readiness['has_risk_assessment'] * 0.1, readiness['has_metrics'] * 0.1])
        return readiness
    ...
    def to_pitch_deck_brief(self) -> dict[str, Any]:
        ...
        """
        return {'company_name': self.name, 'tagline': self.tagline, 'problem': {'description': self.problem.description, 'pain_points': self.problem.pain_points, 'affected_users': self.problem.affected_users}, 'solution': {'description': self.solution.description, 'key_features': self.solution.key_features, 'value_proposition': self.solution.value_proposition}, 'market': {'size': self.market_research.total_addressable_market if self.market_research else None, 'growth_rate': self.market_research.growth_rate if self.market_research else None, 'trends': self.market_research.market_trends if self.market_research else []}, 'business_model': {'revenue_streams': self.business_model.revenue_streams if self.business_model else [], 'key_metrics': self.business_model.metrics if self.business_model else []}, 'competition': [{'name': comp.competitor_name, 'strengths': comp.strengths, 'weaknesses': comp.weaknesses} for comp in self.competitor_analyses[:3]], 'metrics': self.metrics.model_dump() if self.metrics else None}
...
class IdeaPortfolio(BaseModel):
    ...
    def get_top_ideas(self, n: int=3) -> list[StartupIdea]:
        ...
        scored_ideas = [idea for idea in self.ideas if idea.metrics and idea.metrics.overall_score]
        return sorted(scored_ideas, key=lambda x: x.metrics.overall_score, reverse=True)[:n]
    ...
    def get_portfolio_summary(self) -> dict[str, Any]:
        ...
        return {'total_ideas': len(self.ideas), 'ideas_by_stage': {stage.value: len(self.get_ideas_by_stage(stage)) for stage in IdeaStage}, 'ideas_by_category': {cat.value: sum((1 for idea in self.ideas if idea.category == cat)) for cat in IdeaCategory}, 'validated_ideas': len([idea for idea in self.ideas if idea.stage in [IdeaStage.VALIDATED, IdeaStage.PROTOTYPED, IdeaStage.MVP_READY]]), 'average_score': sum((idea.metrics.overall_score for idea in self.ideas if idea.metrics and idea.metrics.overall_score)) / max(len(self.ideas), 1)}

def create_problem_from_description(description: str, severity: str='high') -> ProblemStatement:
    """Create a problem statement from a simple description."""
    return ProblemStatement(problem_id=str(uuid.uuid4()), description=description, affected_users=['To be determined'], frequency='daily', severity=severity, pain_points=['To be researched'])

def create_basic_idea(name: str, problem: str, solution: str, category: IdeaCategory) -> StartupIdea:
    """Create a basic startup idea from minimal information."""
    problem_statement = create_problem_from_description(problem)
    solution_concept = SolutionConcept(solution_id=str(uuid.uuid4()), name=name, description=solution, key_features=['Feature 1', 'Feature 2', 'Feature 3'], value_proposition='To be defined', unique_aspects=['To be determined'])
    return StartupIdea(idea_id=str(uuid.uuid4()), name=name, tagline=f'{name} - {solution[:50]}', category=category, problem=problem_statement, solution=solution_concept)

class IdeaGenerationRequest(BaseModel):
    """Request model for idea generation."""
    focus_area: str | None = Field(None, description="Specific area to focus on (e.g., 'healthcare', 'education')")
    target_audience: str | None = Field(None, description='Target audience for ideas')
    constraints: list[str] = Field(default_factory=list, description='Any constraints to consider')
    num_ideas: int = Field(default=5, description='Number of ideas to generate')
    inspiration_keywords: list[str] = Field(default_factory=list, description='Keywords for inspiration')
...
class IdeaGenerationResponse(BaseModel):
    ...
    market_trends: list[str] = Field(..., description='Relevant market trends identified')