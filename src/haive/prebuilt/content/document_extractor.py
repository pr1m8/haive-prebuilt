...
prompt = ChatPromptTemplate.from_messages(
    [("system", SYSTEM_PROMPT), ("user", "Text: {input}")]
)


class ExtractedInfo(BaseModel):
    title: str | None = Field(
        None, description="Document or passage title, if present."
    )
    person: str | None = Field(
        None, description="Main person mentioned in the passage, if any."
    )
    organization: str | None = Field(
        None, description="Mentioned organization or institution."
    )
    ...
    summary: str | None = Field(
        None, description="A brief abstract of the passage, if appropriate."
    )
...
document_extractor = create_simple_agent(
    engine=document_extractor_config, name="document_extractor"
)
