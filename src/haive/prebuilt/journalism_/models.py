...
class FactCheckStatement(BaseModel):
    ...
    """
    statement: str = Field(description='The original statement or claim being fact-checked', min_length=10)
    status: Literal['confirmed', 'refuted', 'unverifiable', 'vague'] = Field(description='Verification status of the statement')
    explanation: str = Field(description='Detailed explanation of findings or reason for status', min_length=20)
    suggested_keywords: List[str] = Field(description='Keywords for further research if verification is incomplete', default_factory=list, max_items=5)
    confidence: confloat(ge=0.0, le=1.0) = Field(description='Confidence score in the fact-check result', default=0.8)
    sources: List[Dict[str, str]] = Field(description='Supporting sources used for verification', default_factory=list)
...
class FactCheckResult(BaseModel):
    ...
    """
    statements: List[FactCheckStatement] = Field(description='List of individual fact-check results')
    total_claims: int = Field(description='Total number of claims analyzed', ge=0)
    confirmed_count: int = Field(description='Number of confirmed claims', ge=0, default=0)
    refuted_count: int = Field(description='Number of refuted claims', ge=0, default=0)
    unverifiable_count: int = Field(description='Number of unverifiable claims', ge=0, default=0)
    vague_count: int = Field(description='Number of vague claims', ge=0, default=0)
    overall_credibility: confloat(ge=0.0, le=1.0) = Field(description='Overall credibility score of the article', default=0.0)
    ...
    def calculate_statistics(self) -> 'FactCheckResult':
        ...
        if self.statements:
            ...
            self.confirmed_count = sum((1 for s in self.statements if s.status == 'confirmed'))
            self.refuted_count = sum((1 for s in self.statements if s.status == 'refuted'))
            self.unverifiable_count = sum((1 for s in self.statements if s.status == 'unverifiable'))
            self.vague_count = sum((1 for s in self.statements if s.status == 'vague'))
            if self.total_claims > 0:
                credibility_score = (self.confirmed_count * 1.0 + self.unverifiable_count * 0.5) / self.total_claims
...
class ArticleSummary(BaseModel):
    ...
    """
    main_points: List[str] = Field(description='Main points and events from the article', min_items=3, max_items=7)
    key_people: List[str] = Field(description='Important people mentioned in the article', default_factory=list)
    key_statistics: List[str] = Field(description='Important statistics, numbers, or data points', default_factory=list)
    word_count: int = Field(description='Word count of the original article', ge=0)
    summary_text: str = Field(description='Complete summary in paragraph form (150-200 words)', min_length=100, max_length=300)
...
class ToneAnalysis(BaseModel):
    ...
    """
    overall_tone: Literal['neutral', 'positive', 'negative', 'critical', 'opinionated', 'balanced'] = Field(description='Primary tone of the article')
    sentiment_score: confloat(ge=-1.0, le=1.0) = Field(description='Sentiment score where -1 is negative, 0 is neutral, 1 is positive')
    tone_examples: List[Dict[str, str]] = Field(description='Specific examples from text supporting the tone analysis', min_items=1, max_items=5)
    detected_biases: List[str] = Field(description='List of potential biases detected', default_factory=list)
    objectivity_score: confloat(ge=0.0, le=1.0) = Field(description='Score for journalistic objectivity (0=biased, 1=objective)', default=0.5)
...
class ExtractedQuote(BaseModel):
    ...
    context: str = Field(description='Context in which the quote was said', min_length=10)
    position: Optional[str] = Field(description='Position or title of the speaker', default=None)
    significance: Optional[str] = Field(description='Why this quote is significant to the article', default=None)
...
class QuoteExtractionResult(BaseModel):
    ...
    has_attribution: bool = Field(description='Whether all quotes have proper attribution', default=True)
    ...
    def calculate_quote_stats(self) -> 'QuoteExtractionResult':
        ...
        if self.quotes:
            ...
            self.has_attribution = all((q.speaker and q.speaker.lower() != 'unknown' for q in self.quotes))
        return self
...
class GrammarIssue(BaseModel):
    ...
    """
    issue_type: Literal['grammar', 'spelling', 'punctuation', 'style', 'clarity'] = Field(description='Type of issue found')
    ...
    severity: Literal['minor', 'moderate', 'major'] = Field(description='Severity level of the issue', default='moderate')
    location: Optional[str] = Field(description='Approximate location in the text', default=None)
...
class BiasIndicator(BaseModel):
    ...
    """
    bias_type: Literal['political', 'cultural', 'gender', 'racial', 'economic', 'confirmation', 'selection', 'framing'] = Field(description='Type of bias detected')
    example_text: str = Field(description='Text excerpt showing the bias')
    explanation: str = Field(description='Explanation of why this indicates bias', min_length=20)
    severity: Literal['subtle', 'moderate', 'strong'] = Field(description='Severity of the bias', default='moderate')
    suggestion: Optional[str] = Field(description='Suggestion for more neutral language', default=None)
...
class GrammarBiasReview(BaseModel):
    ...
    """
    grammar_issues: List[GrammarIssue] = Field(description='List of grammar and style issues', default_factory=list)
    bias_indicators: List[BiasIndicator] = Field(description='List of potential bias indicators', default_factory=list)
    overall_quality_score: confloat(ge=0.0, le=1.0) = Field(description='Overall writing quality score', default=0.7)
    readability_score: confloat(ge=0.0, le=1.0) = Field(description='Readability score (0=poor, 1=excellent)', default=0.7)
    bias_score: confloat(ge=0.0, le=1.0) = Field(description='Bias score (0=heavily biased, 1=neutral)', default=0.7)
    recommendations: List[str] = Field(description='General recommendations for improvement', default_factory=list, max_items=5)
...
class JournalismAction(BaseModel):
    ...
    """
    actions: List[Literal['summarization', 'fact-checking', 'tone-analysis', 'quote-extraction', 'grammar-and-bias-review', 'full-report', 'no-action-required', 'invalid']] = Field(description='List of actions to perform')
    priority: Literal['low', 'medium', 'high'] = Field(description='Priority level for the request', default='medium')
...
class SearchResult(BaseModel):
    ...
    relevance_score: confloat(ge=0.0, le=1.0) = Field(description='Relevance to the fact being checked', default=0.5)
    credibility_score: Optional[confloat(ge=0.0, le=1.0)] = Field(description='Credibility score of the source', default=None)
...
class ComprehensiveReport(BaseModel):
    ...
    """
    article_title: Optional[str] = Field(description='Title of the analyzed article', default='Untitled Article')
    analysis_timestamp: datetime = Field(description='When the analysis was performed', default_factory=datetime.now)
    summary: Optional[ArticleSummary] = Field(description='Article summary', default=None)
    fact_check_results: Optional[FactCheckResult] = Field(description='Fact-checking results', default=None)
    tone_analysis: Optional[ToneAnalysis] = Field(description='Tone and sentiment analysis', default=None)
    quotes: Optional[QuoteExtractionResult] = Field(description='Extracted quotes', default=None)
    grammar_bias_review: Optional[GrammarBiasReview] = Field(description='Grammar and bias review', default=None)
    overall_assessment: str = Field(description='Overall assessment of the article', default='')
    recommendations: List[str] = Field(description='Key recommendations for improving the article', default_factory=list)
    ...
    def to_markdown(self) -> str:
        ...
        """
        md_parts = ['# Journalism Analysis Report', f'**Article:** {self.article_title}', f'**Analysis Date:** {self.analysis_timestamp.strftime('%Y-%m-%d %H:%M')}', '\n---\n']
        if self.summary:
        ...
        if self.fact_check_results:
            ...
            md_parts.append(f'**Overall Credibility:** {self.fact_check_results.overall_credibility:.0%}')
            ...
            md_parts.append(f'- Unverifiable: {self.fact_check_results.unverifiable_count}')
        ...
        if self.tone_analysis:
            ...
            md_parts.append(f'**Objectivity Score:** {self.tone_analysis.objectivity_score:.0%}')
        ...
        if self.quotes and self.quotes.quotes:
            ...
            md_parts.append(f'*Found {self.quotes.total_quotes} quotes from {self.quotes.unique_speakers} speakers*\n')
            for quote in self.quotes.quotes[:5]:
        ...
        return '\n'.join(md_parts)
__all__ = ['FactCheckStatement', 'FactCheckResult', 'ArticleSummary', 'ToneAnalysis', 'ExtractedQuote', 'QuoteExtractionResult', 'GrammarIssue', 'BiasIndicator', 'GrammarBiasReview', 'JournalismAction', 'SearchResult', 'ArticleChunk', 'ComprehensiveReport']