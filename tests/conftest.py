...
class MockEngine(Engine):
    """Mock engine for testing with custom I/O."""

    engine_type: EngineType = EngineType.LLM
    id: str = Field(default_factory=lambda: generate_test_id("mock-engine"))
    name: str = Field(default_factory=lambda: f"mock_engine_{uuid.uuid4().hex[:8]}")
...
class MockInvokableEngine(InvokableEngine):
    ...
    name: str = Field(
        default_factory=lambda: f"mock_invokable_engine_{uuid.uuid4().hex[:8]}"
    )
...
class MockNonInvokableEngine(NonInvokableEngine):
    ...
    name: str = Field(
        default_factory=lambda: f"mock_non_invokable_engine_{uuid.uuid4().hex[:8]}"
    )
...
def real_aug_llm_engine() -> AugLLMConfig:
    ...
    base_llm = AzureLLMConfig(
        ...
        name=f"aug_base_llm_{uuid.uuid4().hex[:8]}",
        model="gpt-4o-mini",
    ...
    return AugLLMConfig(
        ...
        name=f"real_aug_llm_{uuid.uuid4().hex[:8]}",
        engine_type=EngineType.LLM,
...
def real_embeddings_engine() -> EmbeddingsEngineConfig:
    ...
    return EmbeddingsEngineConfig(
        ...
        name=f"real_embeddings_{uuid.uuid4().hex[:8]}",
...
def real_vectorstore_engine(
    ...
    return VectorStoreConfig(
        ...
        name=f"real_vectorstore_{uuid.uuid4().hex[:8]}",
...
def real_retriever_engine(
    ...
    return BaseRetrieverConfig(
        ...
        name=f"real_retriever_{uuid.uuid4().hex[:8]}",
...
# Helper functions
def create_runnable(*args, **kwargs):
    """Helper function for creating runnable instances."""
    return lambda x: x


def invoke(engine, data, config=None):
    """Helper function for invoking engines."""
    if hasattr(engine, "invoke"):
        return engine.invoke(data, config)
    return data