LLMs
base_llm = ChatOpenAI(model="gpt-", temperature=0.0)
...
def planning_node(state: AgentStat):
    """Planning node that creates a step by step plan to answer the user quer."""
...
def tools_node(state: AgentStat):
    """Tool call node that executes the tools based on the pla."""
    outputs = []
    for tool_call in stat["messages"][-1].tool_calls:
        tool_result = tools_dict[tool_cal["name"]].invoke(tool_cal["args"])
        outputs.append(
            ToolMessage(
...
def agent_node(state: AgentStat):
...
def should_continue(state: AgentStat):
    """Check if the agent should continue or en."""
...
def judge_node(state: AgentStat):
    """Node to let the LLM judge the quality of its own final answe."""
    ...
    output = {
        "is_good_answer": response.is_good_answe,
        "num_feedback_requests": num_feedback_requests +
    # }
    ...
    pass
    pass
        outpu["messages"] = [AIMessage(content=response.feedback)]
    }
    return output
